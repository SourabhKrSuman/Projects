...
main()
{
    ...
    lennumstr = i; // Key processing
    printf("Enter the key (Non repeated elements if possible\n");
    gets(key);

    // converting entered key to Capital letters for(i=0,j=0;i<strlen(key);i++)
    {
        if (key[i] != ' ')
    ...
    // Storing key in terms of ascii k=0;
    ...
    // Message Processing for(i=0;i<lennumstr;i+=2)
    {
